...
class TreeTagger(TaggerI):
    ...
    def __init__(self, path_to_home=None, language='french', 
        ...
        """
        treetagger_paths = ['C:\Treetagger\cmd, C:\Treetagger\\bin']
    ...
    def tag(self, sentences):
        ...
        if(self._abbr_list is None):
            p = Popen([self._treetagger_bin,_input], shell=False, stdin=PIPE, stdout=PIPE, stderr=PIPE)
        elif(self._abbr_list is not None):
            p = Popen([self._treetagger_bin,self._abbr_list], shell=False, stdin=PIPE, stdout=PIPE, stderr=PIPE)
